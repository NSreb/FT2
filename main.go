...
func (t Training) TrainingInfo() InfoMessage {
	return InfoMessage{
		...
		Distance:     t.distance(),
		Speed:        t.meanSpeed(),
		Calories:     0, // будет переопределено в Running
	}
}
...
func (r Running) Calories() float64 {
	...
	durationHours := r.Duration.Hours()
	if durationHours == 0 {
		return 0
	}
	return ((CaloriesMeanSpeedMultiplier*meanSpeed + CaloriesMeanSpeedShift) * r.Weight / MInKm * durationHours * MinInHours)
}

// TrainingInfo возвращает структуру InfoMessage с информацией о проведенной тренировке.
// Это переопределенный метод TrainingInfo() из Training.
func (r Running) TrainingInfo() InfoMessage {
	infoRunn := r.Training.TrainingInfo()
	infoRunn.Calories = r.Calories()
	return infoRunn
}
...
// Calories возвращает количество потраченных ,
// 0 килокалорий при ходьбе.
// Формула расчета:
...
func (w Walking) Calories() float64 {
	...
	return ((CaloriesWeightMultiplier*w.Weight + (math.Pow(meanSpeed, 2)/w.Height)*CaloriesSpeedHeightMultiplier*w.Weight) * durationHours * MinInHours)
}
...
func (s Swimming) meanSpeed() float64 {
	...
	return float64(s.LengthPool*s.CountPool) / MInKm / durationHours
}
...
func (s Swimming) Calories() float64 {
	...
	durationHours := s.Duration.Hours()
	return (meanSpeed + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * durationHours
}
